...
def add_variables(args: argparse.Namespace, variables: Dict) -> Dict:
    ...
    variables["target_tags"] = ",".join(
        ...
        + (list(args.wandb_tags.split(",")) if args.wandb_tags else [])
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    with open(variables["target_model_config"]) as json_f:
        ...
        model_config["trainer"]["cuda_device"] = "__cuda_device__"
        model_config_str = (
...
class create_test_sweep(WandbParserBase):
    ...
    def add_arguments(
        ...
        subparser.add_argument(
            "--target-config-dir",
            type=Path,
            default=Path("best_models_configs"),
